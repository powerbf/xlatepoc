...
const char* dcnxlate(const char* domain, const char* context, const char* msgid1, const char* msgid2, unsigned long n)
{
	if (skip_translation())
	{
	...
	if (mapped_context != NULL && strlen(mapped_context) != 0)
	{
		...
		translation = dngettext(domain, ctx_msgid1.c_str(), ctx_msgid2.c_str(), n);
		if (translation != NULL)
		{
			if (strcmp(translation, ctx_msgid1.c_str()) == 0 || strcmp(translation, ctx_msgid2.c_str()) == 0)
			{
				translation = NULL;
			}
		}
